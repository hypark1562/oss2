...
def test_file_exists():
    ...
    assert os.path.exists(FILE_PATH), "❌ 전처리된 파일이 없습니다. ETL을 먼저 실행하세요."
...
def test_leakage_removed():
    ...
    assert 'gold_earned' not in df.columns, "❌ Data Leakage! 'gold_earned' 컬럼이 남아있습니다."
...
def test_no_missing_values():
    ...
    assert df[numeric_cols].isnull().sum().sum() == 0, "❌ 결측치(NaN)가 아직 남아있습니다."
